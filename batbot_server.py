...
while True:
	...
	while ftp_logged == False:
		...
		try:
			#FTP protocol to communicate over ethernet with the sbRIO
	...
	#Socket server setup
	s = socket.socket(socket.AF_INET6, socket.SOCK_STREAM) #Initialize
	print("Socket connected.")
	s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) #Set options so that we can reuse the same address

	try:
		s.bind((host, port)) #Binds to the socket
	except socket.error as msg:
		print(msg)

	print("Socket bound.")
	print("Listening for client(s)...")
	s.listen(1) #Listens for a connection request; the value determines how many total connections it can handle
	conn, address = s.accept() #Accept the request
	...
	while True:
		...
		data = conn.recv(1024).decode().strip() #Read binary data from the client, then decode it into a readable format
		...
		if data == "run":
			...
			if ready_signal == "ready": #Reads "ready" from sbRIO
				...
				ftp.retrbinary('RETR sbrio_data', open('datafile.dat', 'wb').write) #Retrieve specified file (datafile) from sbRIO
				print("deleting file")
				...
				with open(data_path + 'datafile.dat', 'rb') as fid: #Open datafile for reading 
						for line in fid:
								conn.send(line) #send data line by line
						time.sleep(0.2)
						conn.send(b'done')
