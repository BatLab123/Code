...
while file_counter < iterations:
	...
	if data == "fileready":
		...
		if maketimevec == True: #Runs once
			for line in csv.reader(open('datafile.dat'), delimiter = '\t', skipinitialspace = True):
				t.append(line[0]) 
				dist.append(float(line[0])*0.343/2)
			...
			#dist = [t*(0.343) for x in t] #Sound travels at 343 m/s
			...
			lp_plot, = ax.plot(dist, lp_current_filtered, label = "L", color = 'r', linestyle = '-')
			rp_plot, = ax1.plot(dist, rp_current_filtered, label = "R", color = 'b', linestyle = '-')			
